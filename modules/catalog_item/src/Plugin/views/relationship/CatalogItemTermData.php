...
namespace Drupal\catalog_item\Plugin\views\relationship;
 ...
 *
 * @ViewsRelationship("catalog_item_term_data")
 */
class CatalogItemTermData extends RelationshipPluginBase {
  ...
  /**
   * Constructs a CatalogItemTermData object.
   *
  ...
  public function query() {
    ...
    if (!array_filter($this->options['vids'])) {
      $catalog_taxonomy_index = $this->query->addTable('catalog_taxonomy_index', $this->relationship);
      $def['left_table'] = $catalog_taxonomy_index;
      $def['left_field'] = 'tid';
      $def['field'] = 'tid';
      ...
      $query->addJoin($def['type'], 'catalog_taxonomy_index', 'ctn', 'ctn.tid = td.tid');
      $query->condition('td.vid', array_filter($this->options['vids']), 'IN');
      $query->addTag('taxonomy_term_access');
      $query->fields('td');
      $query->fields('ctn', ['nid']);
