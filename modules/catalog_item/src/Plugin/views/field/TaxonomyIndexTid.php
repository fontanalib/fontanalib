...
/**
 * Field handler to display all taxonomy terms of a catalog_item.
 *
 * @ingroup views_field_handlers
 *
 * @ViewsField("catalog_taxonomy_index_tid")
 */
class TaxonomyIndexTid extends PrerenderList {
  ...
  /**
   * Constructs a CatalogTaxonomyIndexTid object.
   *
  ...
  public function init(ViewExecutable $view, DisplayPluginBase $display, array &$options = NULL) {
    ...
    if ($view->storage->get('base_table') == 'catalog_item_field_revision') {
      $this->additional_fields['nid'] = ['table' => 'catalog_item_field_revision', 'field' => 'nid'];
    }
    else {
  ...
  public function preRender(&$values) {
    ...
    if ($nids) {
      ...
      /**
       * drupal-8.8.0\core\modules\taxonomy\src\TermStorage.php
       */
      $result = getCatalogItemTerms($nids, $vocabs);

      foreach ($result as $catalog_item_nid => $data) {
        foreach ($data as $tid => $term) {
          $this->items[$catalog_item_nid][$tid]['name'] = \Drupal::service('entity.repository')->getTranslationFromContext($term)->label();
          $this->items[$catalog_item_nid][$tid]['tid'] = $tid;
          $this->items[$catalog_item_nid][$tid]['vocabulary_vid'] = $term->bundle();
          $this->items[$catalog_item_nid][$tid]['vocabulary'] = $vocabularies[$term->bundle()]->label();

          if (!empty($this->options['link_to_taxonomy'])) {
            $this->items[$catalog_item_nid][$tid]['make_link'] = TRUE;
            $this->items[$catalog_item_nid][$tid]['path'] = 'taxonomy/term/' . $tid;
          }
        }
      }
    }
