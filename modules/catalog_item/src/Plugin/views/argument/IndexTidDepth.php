 ...
 *
 * This handler is actually part of the catalog item table and has some restrictions,
 * because it uses a subquery to find catalog items with.
 *
 * @ingroup views_argument_handlers
 *
 * @ViewsArgument("catalog_taxonomy_index_tid_depth")
 */
class IndexTidDepth extends ArgumentPluginBase implements ContainerFactoryPluginInterface {
  ...
  public function buildOptionsForm(&$form, FormStateInterface $form_state) {
    $form['depth'] = [
      ...
      '#description' => $this->t('The depth will match catalog items tagged with terms in the hierarchy. For example, if you have the term "fruit" and a child term "apple", with a depth of 1 (or higher) then filtering for the term "fruit" will get catalog items that are tagged with "apple" as well as "fruit". If negative, the reverse is true; searching for "apple" will also pick up catalog items tagged with "fruit" if depth is -1 (or lower).'),
    ];
  ...
  public function query($group_by = FALSE) {
    ...
    $where = (new Condition('OR'))->condition('ctn.tid', $tids, $operator);
    $last = "ctn";
