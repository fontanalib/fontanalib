...
namespace Drupal\catalog_item\Plugin\Search;
...
/**
 * Handles searching for catalog_item entities using the Search module index.
 *
 * @SearchPlugin(
 *   id = "catalog_item_search",
 *   title = @Translation("Content")
 * )
 */
class CatalogItemSearch extends ConfigurableSearchPluginBase implements AccessibleInterface, SearchIndexingInterface, TrustedCallbackInterface {
  ...
  /**
   * The Renderer service to format the username and catalog_item.
   *
   ...
   *   matched against the {catalog_item_field_data} table.
   * - operator: (optional) OR or AND, defaults to OR.
   *
   * @var array
   */
  protected $advanced = [
    'catalog' => ['column' => 'c.catalog'],
    'language' => ['column' => 'i.langcode'],
    'curator' => ['column' => 'c.uid'],
    'term' => ['column' => 'ti.tid', 'join' => ['table' => 'taxonomy_index', 'alias' => 'ti', 'condition' => 'c.nid = ti.nid']],
  ];
  ...
  /**
   * Constructs a \Drupal\catalog_item\Plugin\Search\CatalogItemSearch object.
   *
  ...
  public function __construct(array $configuration, $plugin_id, $plugin_definition, Connection $database, EntityTypeManagerInterface $entity_type_manager, ModuleHandlerInterface $module_handler, Config $search_settings, LanguageManagerInterface $language_manager, RendererInterface $renderer, MessengerInterface $messenger, AccountInterface $account = NULL, Connection $database_replica = NULL, SearchIndexInterface $search_index = NULL) {
    ...
    $this->addCacheTags(['catalog_item_list']);
    if (!$search_index) {
      @trigger_error('Calling CatalogItemSearch::__construct() without the $search_index argument is deprecated in drupal:8.8.0 and is required in drupal:9.0.0. See https://www.drupal.org/node/3075696', E_USER_DEPRECATED);
  ...
  public function access($operation = 'view', AccountInterface $account = NULL, $return_as_object = FALSE) {
    $result = AccessResult::allowedIfHasPermission($account, 'access catalog');
  ...
  public function isSearchExecutable() {
    // CatalogItem search is executable if we have keywords or an advanced parameter.
  ...
  protected function findResults() {
    ...
    $query->join('catalog_item_field_data', 'c', 'c.nid = i.sid AND c.langcode = i.langcode');
    ...
    $this->addCatalogItemRankings($query);

    // Run the query.
    $find = $query
      // Add the language code of the indexed item to the result of the query,
      // since the catalog_item will be rendered using the respective language.
      ->fields('i', ['langcode'])
  ...
  protected function prepareResults(StatementInterface $found) {
    ...
    $catalog_item_render = $this->entityTypeManager->getViewBuilder('catalog_item');
    $keys = $this->keywords;

    foreach ($found as $item) {
      // Render the catalog_item.
      /** @var \Drupal\catalog_item\CatalogItemInterface $catalog_item */
      $catalog_item = $catalog_item_storage->load($item->sid)->getTranslation($item->langcode);
      $build = $catalog_item_render->view($catalog_item, 'search_result', $item->langcode);

      /** @var \Drupal\catalog_item\CatalogInterface $catalog*/
      $catalog = $this->entityTypeManager->getStorage('catalog')->load($catalog_item->bundle());
      ...
      $rendered .= ' ' . $this->moduleHandler->invoke('comment', 'catalog_item_update_index', [$catalog_item]);

      $extra = $this->moduleHandler->invokeAll('catalog_item_search_result', [$catalog_item]);

      $username = [
        '#theme' => 'username',
        '#account' => $catalog_item->getOwner(),
      ];

      $result = [
        'link' => $catalog_item->toUrl('canonical', ['absolute' => TRUE])->toString(),
      ...
      $this->addCacheableDependency($catalog_item);

      // We have to separately add the catalog_item owner's cache tags because search
      ...
      // explicitly here.
      $this->addCacheableDependency($catalog_item->getOwner());

      if ($catalog->displaySubmitted()) {
        $result += [
          'user' => $this->renderer->renderPlain($username),
          'date' => $catalog_item->getChangedTime(),
        ];
      }
   ...
   *
   * This information is being removed from the rendered catalog_item that is used to
  ...
  public function updateIndex() {
    // Interpret the cron limit setting as the maximum number of catalog_items to index
    // per cron run.
    $limit = (int) $this->searchSettings->get('index.cron_limit');

    $query = $this->databaseReplica->select('catalog_item', 'c');
    $query->addField('c', 'nid');
    $query->leftJoin('search_dataset', 'sd', 'sd.sid = c.nid AND sd.type = :type', [':type' => $this->getPluginId()]);
    ...
    $words = [];
    try {
      foreach ($catalog_item_storage->loadMultiple($nids) as $catalog_item) {
        $words += $this->indexCatalogItem($catalog_item);
      }
    }
    finally {
  ...
  /**
   * Indexes a single catalog_item.
   *
   * @param \Drupal\catalog_item\CatalogItemInterface $catalog_item
   *   The catalog_item to index.
   *
   * @return array
   *   An array of words to update after indexing.
   */
  protected function indexCatalogItem(CatalogItemInterface $catalog_item) {
    $words = [];
    $languages = $catalog_item->getTranslationLanguages();
    $catalog_item_render = $this->entityTypeManager->getViewBuilder('catalog_item');

    foreach ($languages as $language) {
      $catalog_item = $catalog_item->getTranslation($language->getId());
      // Render the catalog_item.
      $build = $catalog_item_render->view($catalog_item, 'search_index', $language->getId());
      ...
      $extra = $this->moduleHandler->invokeAll('catalog_item_update_index', [$catalog_item]);
      foreach ($extra as $t) {
        $text .= $t;
      }

      // Update index, using search index "type" equal to the plugin ID.
      $words += $this->searchIndex->index($this->getPluginId(), $catalog_item->id(), $language->getId(), $text, FALSE);
    }
    return $words;
  }
  ...
  public function indexClear() {
    // All CatalogItemSearch pages share a common search index "type" equal to
    // the plugin ID.
  ...
  public function indexStatus() {
    $total = $this->database->query('SELECT COUNT(*) FROM {catalog_item}')->fetchField();
    $remaining = $this->database->query("SELECT COUNT(DISTINCT c.nid) FROM {catalog_item} c LEFT JOIN {search_dataset} sd ON sd.sid = c.nid AND sd.type = :type WHERE sd.sid IS NULL OR sd.reindex <> 0", [':type' => $this->getPluginId()])->fetchField();
  ...
  public function searchFormAlter(array &$form, FormStateInterface $form_state) {
    ...
    // Add catalogs.
    $catalogs = array_map(['\Drupal\Component\Utility\Html', 'escape'], catalog_get_names());
    $form['advanced']['catalogs-fieldset'] = [
      '#type' => 'fieldset',
      '#title' => t('Catalogs'),
    ];
    $form['advanced']['catalogs-fieldset']['catalog'] = [
      '#type' => 'checkboxes',
      '#title' => t('Only in the catalog(s)'),
      '#prefix' => '<div class="criterion">',
      '#suffix' => '</div>',
      '#options' => $catalogs,
      '#default_value' => isset($defaults['catalog']) ? $defaults['catalog'] : [],
    ];
  ...
  public function buildSearchUrlQuery(FormStateInterface $form_state) {
    ...
    $filters = [];
    if ($form_state->hasValue('catalog') && is_array($form_state->getValue('catalog'))) {
      // Retrieve selected types - Form API sets the value of unselected
      // checkboxes to 0.
      foreach ($form_state->getValue('catalog') as $type) {
        if ($type) {
          $advanced = TRUE;
