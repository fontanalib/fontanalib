...
/**
 * Defines a controller to render a single catalog_item.
 */
class CatalogItemViewController extends EntityViewController {
  ...
  public function __construct(EntityTypeManagerInterface $entity_type_manager, RendererInterface $renderer, AccountInterface $current_user = NULL, EntityRepositoryInterface $entity_repository = NULL) {
    ...
    if (!$entity_repository) {
      @trigger_error('The entity.repository service must be passed to CatalogItemViewController::__construct(), it is required before Drupal 9.0.0. See https://www.drupal.org/node/2549139.', E_USER_DEPRECATED);
   ...
   * {@inheritdoc}
   */
  public function view(EntityInterface $catalog_item, $view_mode = 'full', $langcode = NULL) {
    $build = parent::view($catalog_item, $view_mode, $langcode);

    foreach ($catalog_item->uriRelationships() as $rel) {
      $url = $catalog_item->toUrl($rel)->setAbsolute(TRUE);
      ...
      if ($this->currentUser->isAuthenticated() || $url->access($this->currentUser)) {
        // Set the catalog_item path as the canonical URL to prevent duplicate content.
  ...
  /**
   * The _title_callback for the page that renders a single catalog_item.
   *
   ...
   *   The page title.
   */
  public function title(EntityInterface $catalog_item) {
    return $this->entityRepository->getTranslationFromContext($catalog_item)->label();
  }
